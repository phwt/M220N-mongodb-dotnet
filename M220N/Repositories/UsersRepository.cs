        ...
        */
        ...
        public async Task<UserResponse> LoginUserAsync(User user, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                var filterDefinition = Builders<Session>.Filter.Eq(s => s.UserId, user.Email);
                var updateDefinition = Builders<Session>.Update.Set(s => s.UserId, user.Email)
                                                     .Set(s => s.Jwt, user.AuthToken);
                var updateOptions = new UpdateOptions() { IsUpsert = true };

                await _sessionsCollection.UpdateOneAsync(filterDefinition, updateDefinition, updateOptions);
        ...
        public async Task<UserResponse> LogoutUserAsync(string email, CancellationToken cancellationToken = default)
        {
            ...
            await _sessionsCollection.DeleteOneAsync(Builders<Session>.Filter.Eq(s => s.UserId, email), cancellationToken);
        ...
        public async Task<Session> GetUserSessionAsync(string email, CancellationToken cancellationToken = default)
        {
            ...
            return await _sessionsCollection.Find(Builders<Session>.Filter.Eq(s => s.UserId, email)).FirstOrDefaultAsync();
        }
